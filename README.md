...
`src/main.cpp` 를 업로드하면 보드의 `L` LED 가 0.5초 간격으로 점멸합니다.
이것만 되면 **컴파일과 업로드**는 정상입니다.

**시리얼 통신까지** 한 번에 확인하려면 `examples/00_env_check.cpp` 의 내용을
`src/main.cpp` 에 덮어쓰고 업로드하세요. 다음 세 가지가 모두 동작해야 합니다.
